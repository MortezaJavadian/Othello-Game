...
int GameBackground(info GameInfo, info copy[2])
{
    ...
    do
    {
        ...
        else if (movement == Esc)
        {
            ...
            SaveFile = fopen("Unfinished.json", "a+");
            ToJson(SaveFile, GameInfo, NameFile);
            ToJson(SaveFile, copy[0], NameFile);
            ToJson(SaveFile, copy[1], NameFile);
            fclose(SaveFile);

            SaveFile = fopen("Unfinished.json", "r");
